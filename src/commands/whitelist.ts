...
export default async function(player: Player, args: string[]) {
	...
	switch (subcommand) {
		...
		case "add": {
			...
			const target = args[0];
			if (!target) {
				bot.chat(`/msg ${ player.username } Usage: ${ COMMAND_CHAT_PREFIX }whitelist add <username>`);
				break;
			}
		...
		case "rm": {
			...
			const target = args[0];
			if (!target) {
				bot.chat(`/msg ${ player.username } Usage: ${ COMMAND_CHAT_PREFIX }whitelist rm <username>`);
				break;
			}
			const targetPlayer = Object.values(bot.players).find(e => e.username === target);
			if (!targetPlayer || !targetPlayer.uuid) {
