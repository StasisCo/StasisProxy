	...
	program
		...
		.action(async(code: string) => {
			...
			if (interactionPayload.success) {
				await DiscordClient.client.rest.delete(Routes.webhookMessage(interactionPayload.data.applicationId, interactionPayload.data.token, "@original")).catch(() => {});
			} else {
				...
				if (legacyPayload.success) {
					const guild = await DiscordClient.client.guilds.fetch(legacyPayload.data.guildId).catch(() => null);
