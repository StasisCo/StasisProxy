 ...
 */
export class SpectatorHologram {
	...
	private async spawn(pearl: Pearl) {
		...
		const entityId = SpectatorHologram.nextEntityId++;
		...
		this.client.writeRaw(proto.createPacketBuffer("packet", {
			name: "teams",
			params: {
				team: SpectatorHologram.HIDDEN_NAMETAG_TEAM,
				mode: 3, // add players
				players: [ fakeName ]
			}
		}));
		...
		const nametagY = column.surfaceY + 2; // ~ above player's head
		...
		for (let i = 0; i < lines.length; i++) {
			const lineEntityId = SpectatorHologram.nextEntityId++;
	...
	public handleCloseWindow(data: { windowId: number }): boolean {
		if (data.windowId !== SpectatorHologram.WINDOW_ID || this.openWindowId === null) return false;
	...
	private openContainer(entry: { ownerName: string }) {
		const proto = this.client.serializer.proto;
		const windowId = SpectatorHologram.WINDOW_ID;
